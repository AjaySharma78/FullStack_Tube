...
router.route("/register").post(upload.fields([{ name: "avatar", maxCount: 1 },{ name: "coverImage", maxCount: 1 },]),registerUsers);
...
router.route("/update/u/email-username-fullname").patch(verifyUser, updateUserEmailUsername);
router.route("/user-channel-profile").get(verifyUser, getloginUserChannelProfile);
...
router.get("/auth/google/callback",passport.authenticate("google", { failureRedirect: "/login" }),googleOAuthCallback);

router.get("/auth/github",passport.authenticate("github", { scope: ["user:email"] }));
router.get("/auth/github/callback",passport.authenticate("github", { failureRedirect: "/login" }),githubOAuthCallback);
